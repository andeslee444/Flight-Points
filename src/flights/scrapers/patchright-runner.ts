/**
 * Shared Patchright Python subprocess runner.
 *
 * Mirrors camoufox-runner.ts but for Patchright-based scrapers.
...
export interface PatchrightRunnerOptions {
  /** Display name for logging, e.g. 'AA-Patchright' */
  label: string;
  /** Python script filename (relative to scrapers dir), e.g. 'aa-patchright.py' */
  script: string;
...
/**
 * Run a single Patchright search with caching and retry.
 */
export function runPatchrightSearch(
  params: SearchParams,
  opts: PatchrightRunnerOptions,
 ...
 */
export async function runPatchrightBatch(
  ...
  const doSearch = searchFn || ((p: SearchParams) => runPatchrightSearch(p, opts));
