#!/usr/bin/env python3
"""
ANA Award Search via Patchright (patched Chromium).

Same logic as ana-camoufox.py but uses Patchright. Adds a Patchright
variant as a fallback option (rate limiting is still the main issue).

Usage:
  python3 ana-patchright.py '{"origin":"JFK","destination":"NRT","date":"2026-03-15","cabin":"business"}'
...
def log(msg):
    print(f"[ANA-Patchright {time.strftime('%H:%M:%S')}] {msg}", file=sys.stderr)
...
def main():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # Step 1: Extended warmup — browse ANA site to establish session & bypass WAF
            log("Warming up on ana.co.jp (extended)...")
            try:
                page.goto("https://www.ana.co.jp/en/us/", timeout=30000, wait_until="domcontentloaded")
                random_delay(3, 6)

                # Dismiss cookie consent early
                for sel in ['#ensCloseBanner', 'button:has-text("Accept")',
                    ...
                    try:
                        ...
                        if btn and btn.is_visible():
                            btn.click()
                            log("Dismissed homepage cookie consent")
                            random_delay(1, 2)
                            break
                    except:
                        continue

                # Simulate browsing — scroll, move mouse, click a section
                ...
                # Try navigating to award booking through the UI links
                try:
                    # Look for "ANA Mileage Club" or "Use Miles" links
                    for link_sel in ['a:has-text("ANA Mileage Club")', 'a:has-text("Use Miles")',
                                     'a:has-text("Award Reservation")', 'a[href*="award"]',
                                     'a[href*="mileage"]']:
                        link = page.query_selector(link_sel)
                        if link and link.is_visible():
                            log(f"Found navigation link: {link_sel}")
                            # Just note it exists, we'll navigate via URL below
                            break
                except:
                    pass
...
def try_booking_search(page, username, password, origin, destination, date, cabin):
    ...
    try:
        ...
        # Login if needed (with extended human-like delays to avoid WAF)
        acct_field = page.query_selector("#accountNumber")
        if acct_field:
            log("Login required, filling credentials...")
            # Extra pre-login delay and mouse activity
            random_delay(2, 4)
            ...
            acct_field.click()
            random_delay(0.5, 1)
            # Type slowly with variable delays
            acct_field.type(username, delay=random.randint(80, 160))
            random_delay(1.5, 3)
            ...
            if pw_field:
                ...
                pw_field.click()
                random_delay(0.5, 1)
                pw_field.type(password, delay=random.randint(70, 140))
                random_delay(1, 2)

            # Small delay before clicking login (like a human would pause)
            random_delay(1, 3)
            try:
        ...
        # Select One-Way
        try:
            for sel in ['input[value="2"]', 'label:has-text("One-way")', 'input[id*="oneWay"]', '#oneWay']:
        ...
        # Hide header elements that intercept clicks
        page.evaluate("""() => {
