#!/usr/bin/env python3
"""
Air France / KLM Flying Blue Award Search via Patchright (patched Chromium).

No login required. Shows SkyTeam partner availability.
Uses Patchright to bypass Akamai that blocks stock Playwright.

Usage:
  python3 flying-blue-patchright.py '{"origin":"JFK","destination":"CDG","date":"2026-04-15","cabin":"business"}'
...
def log(msg):
    print(f"[FlyingBlue-PR {time.strftime('%H:%M:%S')}] {msg}", file=sys.stderr)
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            def handle_response(response):
                ...
                # Track all API-like responses for debugging
                ...
                if "json" in ct and status == 200:
                    try:
                        ...
                        if len(body) > 100:
                            ...
                            # Capture any response that could contain flight data
            ...
            # Cookie warming
            log("Warming cookies on airfrance.us...")
            try:
                page.goto("https://www.airfrance.us/", timeout=20000, wait_until="domcontentloaded")
            ...
            # Navigate to search (try airfrance.us first, then KLM fallback)
            ...
            # Check if we got intercepted API data first
            ...
            if not results:
                ...
                if "captcha" in content.lower() or "challenge" in content.lower():
                    log("Blocked by anti-bot protection on airfrance.us")

                    # Try direct GraphQL call from homepage context
                    log("Trying direct GraphQL search from homepage...")
                    try:
                        page.goto("https://www.airfrance.us/", timeout=30000, wait_until="domcontentloaded")
                        time.sleep(random.uniform(3, 5))

                        gql_result = page.evaluate("""({origin, destination, date, cabin}) => {
                            const cabinMap = {economy: 'ECONOMY', business: 'BUSINESS', first: 'FIRST'};
                            return fetch('https://wwws.airfrance.us/gql/v1?bookingFlow=LEISURE&brand=AF&country=US&language=en', {
                                method: 'POST',
                                headers: {
                                ...
                                body: JSON.stringify({
                                    ...
                                    variables: {
                                        searchCriteria: {
                                            ...
                                            connections: [{origin, destination, outboundDate: date}],
                                            tripType: 'ONE_WAY',
                                        }
                                    },
                                    query: 'query SearchOffersQuery($searchCriteria: SearchCriteriaInput!) { searchOffers(searchCriteria: $searchCriteria) { connections { segments { flightNumber airline departure arrival duration stops } milesPrice { miles value } taxPrice { value currency } cabin fareBasis } } }'
                                }),
                                credentials: 'include',
                            }).then(r => {
                        ...
                        if gql_result and gql_result.get('ok'):
                            body_text = gql_result.get('body', '')
                            log(f"Direct GraphQL response: {len(body_text)} bytes")
                            try:
                                data = json.loads(body_text)
                                if "errors" in data:
                                    log(f"GraphQL error: {json.dumps(data['errors'][:1])[:200]}")
                                else:
                                    results = parse_api_flights(data, origin, destination, date, cabin, search_url)
                                    if results:
                                        log(f"Got {len(results)} results from direct GraphQL")
                            except json.JSONDecodeError:
                                log(f"Not JSON: {body_text[:200]}")
                        else:
                            err = gql_result.get('error', 'unknown') if gql_result else 'null'
                            log(f"Direct GraphQL failed: {err}")
                    except Exception as e:
                        log(f"Direct GraphQL approach error: {str(e)[:100]}")

                if not results:
                    # Try KLM as fallback (same Flying Blue backend, different Akamai config)
                    ...
                    try:
                        ...
                        # Check intercepted API data from KLM
                        if api_flights:
                            for api_data in api_flights:
                                try:
                    ...
                    if not results:
                        log(f"Both airfrance.us and klm.com blocked")
                        if all_api_urls:
...
def parse_api_flights(data, origin, destination, date, cabin, booking_url):
    ...
    results = []
    try:
        # Try various API response structures (AF/KLM use different schemas)
        connections = (
