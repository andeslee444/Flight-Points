...
const SCRIPT_PATH = resolve(__dirname, 'ana-camoufox.py');
const TIMEOUT_MS = 180_000; // 3 minutes (ANA is slower than AA)

function log(msg: string) {
  console.log(`[ANA-Camoufox-TS ${new Date().toISOString().slice(11, 23)}] ${msg}`);
}

export function searchANACamoufox(params: SearchParams): Promise<FlightResult[]> {
  const cacheKey = getCacheKey('ana', params);
...
export async function searchANACamoufoxBatch(
  ...
  for (let i = 0; i < paramsList.length; i++) {
    ...
    const flights = await searchANACamoufox(params);
